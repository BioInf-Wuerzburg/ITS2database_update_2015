...
path to the desired output file to write a list of all taxids of the lowest rank
		
=cut		
		
$options{'taxids=s'} = \(my $opt_taxids);
...
foreach my $gi (keys %gi_taxonomy){
    my %tax_elements;
    $taxids{$gi} = $gi_taxonomy{$gi}->[0]->{taxid};
...
if($opt_taxids){
    open(ST, ">", $opt_taxids) or die "$opt_taxids $!";
    foreach(keys %taxids){
	print ST "$_\t$taxids{$_}\n";
    }
    close ST or die "$!";
